...
for epoch in tqdm(range(resume_epochs + 1, n_epochs + 1)):
    ...
    loss_sum = 0
    loss_iter = 0
    for train_input, reference in training_generator:
        # scheduler.step()
        ...
        loss = criterion(output.view(-1), reference.view(-1))
    ...
    if epoch % validation_interval == 0 and epoch != 0:
        model.eval()
        with torch.no_grad():
            for validation_input, validation_y in validation_generator:
                validation_input = validation_input.to(device)
                validation_y = validation_y.to(device)
                validation_output = model(validation_input)

                val_loss = criterion(validation_output.view(-1), validation_y.view(-1))
            ...
            if patience == max_patience:
                ...
                if num_trial == max_trial:
                    ...
                    for i in range(len(hist_epoch)):
                        tqdm.write(str(hist_epoch[i]) + ', ' + str(hist_training_loss[i]) + ', ' + str(
                            hist_validation_loss[i]))
                        hist_file.write(
